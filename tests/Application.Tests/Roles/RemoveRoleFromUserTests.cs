...
public class RemoveRoleFromUserTests
{
    ...
    public RemoveRoleFromUserTests() =>
        _handler = new RemoveRoleFromUserCommandHandler(
            ...
            Mock.Of<ILogger<RemoveRoleFromUserCommandHandler>>()
        );

    [Fact]
    public async Task Handle_WhenAdmin_RemovesRoleAndReturnsUserId()
    {
        // Arrange
        ...
        _mockRoleRepo.Verify(
            r => r.RemoveFromUserAsync(roleId, userId, It.IsAny<CancellationToken>()),
            Times.Once
        );
    }
    ...
    public async Task Handle_WhenNotAdmin_ReturnsForbidden()
    {
        ...
        // Act
        Result<Guid> result = await _handler.Handle(
            new RemoveRoleFromUserCommand(Guid.NewGuid(), Guid.NewGuid()),
            CancellationToken.None
        );
    ...
    public async Task Handle_WhenRoleNotFound_ReturnsNotFound()
    {
        ...
        // Act
        Result<Guid> result = await _handler.Handle(
            new RemoveRoleFromUserCommand(roleId, Guid.NewGuid()),
            CancellationToken.None
        );
